...
class ConditionalResidualBlock1D(nn.Module):
    def __init__(self, 
        ...
        self.bx = nn.Linear(out_channels,emb_size)
        self.bx1 = nn.Linear(out_channels,emb_size)
        self.emd1 = nn.Sequential(
            nn.Linear(cond_dim, emb_size),
            nn.ReLU(),
        ...
        self.linn = nn.Linear(emb_size,out_channels)
    ...
    def forward(self, x, cond):
       ...
       # print("out",out.size())
        ...
        for dit in self.dits:
            out=dit(out,emd1)
        #print("out.size1", out.size())
        out = self.ln(out)
        #print("out.size2", out.size())
        #print("outsize emd1size",out.size(),emd1.size())
        out = self.linn(out)
        #print("out.size3", out.size())
        out = out.permute(0,2,1)
        #print("out.size4", out.size())
        #print("out1,x", out.size(),self.residual_conv(x).size())
...
class ConditionalUnet1D(nn.Module):
    ...
    def forward(self, 
        ...
        x = sample
        #print(x.size(), global_feature.size(), global_cond.size())
        h = []
        #print("first",x.size())
        ...
        '''lsr = h.pop()
        ppp = torch.cat((x, lsr), dim=1)
        print(ppp.size())
        print("middle: ",x.size(),lsr.size())
        print("neirong",x,lsr)
        print(f"x dtype: {x.dtype}, h[-1] dtype: {lsr.dtype}")'''
        #lsr = h.pop()
        #ppp = torch.cat((x, lsr), dim=1)
        #print("ppp",ppp.size())
        for idx, (resnet, resnet2, upsample) in enumerate(self.up_modules):
            lsr = h.pop()
            #print("zhiqian",lsr.size(), x.size())
            x = torch.cat((x, lsr), dim=1)

            x = resnet(x, global_feature)
            x = resnet2(x, global_feature)
            x = upsample(x)
            #print("zhihou",lsr.size(),x.size())
        #print("second", x.size())
        x = self.final_conv(x)
        #print("third", x.size())
        # (B,C,T)
