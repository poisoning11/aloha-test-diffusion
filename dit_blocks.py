...
class DITBLOCKS(nn.Module):
    ...
    def forward(self, x, cond):  # (batch_size fenkuai*fenkuai emb_size) cond (batch_size emb_size)
        ...
        wk = self.wk(x)
        wv = self.wv(x)
        wq = wq.view(wq.size(0), wq.size(1), self.nhead, -1).permute(0, 2, 1, 3)
        wk = wk.view(wk.size(0), wk.size(1), self.nhead, -1).permute(0, 2, 3, 1)
        wv = wv.view(wv.size(0), wv.size(1), self.nhead, -1).permute(0, 2, 1, 3)
        attn = wq @ wk/math.sqrt(wq.size(2))
        attn = torch.softmax(attn,dim=-1)
        attn = attn @ wv
        attn  = attn.permute(0,2,1,3)
        attn = attn.reshape(attn.size(0),attn.size(1),attn.size(2)*attn.size(3))
        x = self.linear1(attn)
        x = x*ap1.unsqueeze(1)
        x = x + y
        z = x
